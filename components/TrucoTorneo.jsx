...
import { Lock, Unlock, X, Plus, Minus, Trophy } from "lucide-react";
...
export default function TrucoTorneoDemo() {
  ...
  function adjustScore(id, side, delta) {
    setMesas((prev) =>
      prev.map((m) =>
        m.id === id
          ? { ...m, [side]: Math.max(0, Math.min(30, m[side] + delta)) }
          : m
      )
    );
  }
  ...
  function pickWinner(round, idx, name) {
    if (!isAdmin || name === "TBD") return;
    setBracket((prev) => {
      const next = structuredClone(prev);
      next[round][idx].winner = next[round][idx].winner === name ? null : name;
      return next;
    });
  }
        ...
        {tab === "posiciones" && (
          <div className="tw-table-wrap">
            <table className="tw-standings">
              <thead>
                <tr><th>#</th><th>Pareja</th><th>PJ</th><th>PG</th><th>PP</th><th>Pts</th></tr>
              </thead>
              <tbody>
                {initialStandings.map((s, i) => (
                  <tr key={s.pair}>
