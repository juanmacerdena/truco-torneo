...
function normalizeStandings(value) {
  return Array.isArray(value) ? value : initialStandings;
}
...
function normalizeBracket(value) {
  ...
  return {
    cuartos: Array.isArray(value.cuartos) ? value.cuartos : initialBracket.cuartos,
    semis: Array.isArray(value.semis) ? value.semis : initialBracket.semis,
    final: Array.isArray(value.final) ? value.final : initialBracket.final,
  };
}
...
export default function TrucoTorneoDemo() {
  ...
  const safeMesas = normalizeMesas(mesas);
  ...
  async function adjustScore(id, side, delta) {
    const mesa = safeMesas.find((m) => m.id === id);
    if (!mesa) return;
    const currentScore = toNumber(mesa[side], 0);
    const newScore = Math.max(0, Math.min(30, currentScore + delta));
    ...
    try {
      ...
      setMesas((prev) => prev.map((m) => (m.id === id ? normalizeMesa(updated, id) : m)));
    } catch (err) {
  ...
  async function pickWinner(round, idx, name) {
    ...
    const nextBracket = structuredClone(safeBracket);
    ...
    try {
      ...
      const updated = await res.json();
      setBracket(normalizeBracket(updated));
    } catch (err) {
  ...
  async function createMesa(e) {
    ...
    try {
      ...
      setMesas((prev) => [...normalizeMesas(prev), normalizeMesa(created, normalizeMesas(prev).length + 1)]);
  ...
  async function deleteMesa(id) {
    setConfirmDialog({
      title: "Eliminar mesa",
      message: "Esta mesa se va a borrar de forma permanente. Esta acción no se puede deshacer.",
      ...
      onConfirm: async () => {
        try {
          ...
          setMesas((prev) => normalizeMesas(prev).filter((m) => m.id !== id));
        } catch (err) {
          console.error(err);
  ...
  async function deleteParticipant(id) {
    setConfirmDialog({
      ...
      message: "La pareja se va a borrar de forma permanente. Esta acción no se puede deshacer.",
      ...
      onConfirm: async () => {
        try {
          ...
          setStandings((prev) => normalizeStandings(prev).filter((p) => p.id !== id));
        } catch (err) {
          console.error(err);
  ...
  async function createParticipant(e) {
    ...
    try {
      ...
      const created = await res.json();
      setStandings((prev) => [...normalizeStandings(prev), normalizeStanding(created, normalizeStandings(prev).length)].sort((a, b) => b.pts - a.pts));
  ...
  async function updateParticipant(e) {
    ...
    try {
      ...
      const updated = await res.json();
      setStandings((prev) =>
        normalizeStandings(prev)
          .map((p) => (p.id === editingPart.id ? normalizeStanding(updated, editingPart.id - 1) : p))
  ...
  return (
    <div className="tw-root">
      <style>{`
        ...
        .tw-footer { margin-top: 28px; padding: 16px 0 4px; text-align: center; color: var(--cream-dim); font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; }
          ...
          <>
            ...
            {tab === "cruces" && (
              <div className="tw-bracket">
                {BRACKET_ROUNDS.map(([roundKey, label]) => {
                  const matches = Array.isArray(safeBracket[roundKey]) ? safeBracket[roundKey] : [];

                  return (
                    <div className="tw-round" key={roundKey}>
                      <div className="tw-round-label">{label}</div>
      ...
      {confirmDialog && (
        <div className="tw-modal-overlay" onClick={() => setConfirmDialog(null)}>
          <div className="tw-modal" style={{ maxWidth: 460 }} onClick={(e) => e.stopPropagation()}>
            <div className="tw-modal-close" onClick={() => setConfirmDialog(null)}><X size={18} /></div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <div className="tw-display" style={{ fontSize: 21, fontWeight: 600 }}>{confirmDialog.title}</div>
            </div>
            <div style={{ fontSize: 14, lineHeight: 1.6, color: "var(--cream-dim)" }}>
              {confirmDialog.message}
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 22, flexWrap: "wrap" }}>
              <button className="tw-btn tw-focus" onClick={() => setConfirmDialog(null)}>
                Cancelar
              </button>
              <button
                className="tw-btn solid tw-focus"
                style={confirmDialog.tone === "danger" ? { background: "#E9998D", borderColor: "#E9998D", color: "#201F1D" } : undefined}
      ...
      <footer className="tw-footer">by CERDENA SAS</footer>
    </div>
  );
}
